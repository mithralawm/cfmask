...
!Design Notes:
  1. The following public functions handle the input data:
     OpenInput - Setup 'input' data structure, open file for access, read the
     metadata, and allocate space for memory.
  ...
  while(fgets(buffer, MAX_STR_LEN, in) != NULL) {
   ...
   if (s != NULL)
    ...
    label=trimwhitespace(tokenptr);
      ...
      if (strcmp(label,"PROJECTION_ZONE") == 0)
      {
          tokenptr = trimwhitespace(strtok(NULL, seperator));
          printf("label = %s\n",label);
          printf("tokenptr = %s\n",tokenptr);
...
Input_t *OpenInput(char *lndsr_name, char *lndcal_name, char *lndmeta_name)
{
  ...
  this->lndsr_name = DupString(lndsr_name);
  if (this->lndsr_name == NULL) {
    free(this);
  ...
  this->sds_sr_file_id = SDstart((char *)lndsr_name, DFACC_RDONLY);
  if (this->sds_sr_file_id == HDF_ERROR) {
    free(this->lndsr_name);
    free(this->lndcal_name);
    free(this);  
    RETURN_ERROR("opening lndsr input file", "OpenInput", NULL); 
  }
  /* Open file for SD access */
...
bool GetInputMeta(Input_t *this) 
{
  ...
  if (!GetAttrDouble(this->sds_sr_file_id, &attr, dval))
    RETURN_ERROR("reading attribute (West Bounding Coords)", "GetInputMeta", false);
  if (attr.nval != 1) 
    RETURN_ERROR("invalid number of values (West Bounding Coords)", 
                 "GetInputMeta", false);
  meta->ul_lon = (float)(dval[0] / DEG);
  if (meta->ul_lon < -90.0  || meta->ul_lon  > 90.0) 
    RETURN_ERROR("West Bounding Coords out of range", "GetInputMeta", false);

  attr.type = DFNT_FLOAT64;
  attr.nval = 1;
  attr.name = INPUT_NORTH_BOUND;
  if (!GetAttrDouble(this->sds_sr_file_id, &attr, dval))
    RETURN_ERROR("reading attribute (North Bounding Coords)", "GetInputMeta", false);
  if (attr.nval != 1) 
    RETURN_ERROR("invalid number of values (North Bounding Coords)", 
                 "GetInputMeta", false);
  meta->ul_lat = (float)(dval[0] / DEG);
  if (meta->ul_lat < -180.0  || meta->ul_lat  > 180.0) 
    RETURN_ERROR("North Bounding Coords out of range", "GetInputMeta", false);
...
bool GetInputMeta2(Input_t *this) 
{
  ...
  this->meta.satu_value_ref = (float)dval[0];
